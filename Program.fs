...
type ProcResult = { out: string; err: string; code: int }
...
let twitchKey =
    ...
    | null -> failwithf "Missing TWITCH_STREAM_KEY"
    | x -> x
...
let twitch cmd =
    ...
    let parseStatus out =
        Regex.Match(out, ".+fps=(?<fps>[0-9 .]+).+size=(?<size>.+)time=(?<time>[0-9:.]+).*bitrate=(?<bitrate>.+/s).")
    ...
    let handleProgress =
        ...
        fun out ->
            ...
            | capture when capture.Success ->
                if started = false && trim capture.Groups.["size"].Value <> "0kb" then
                    started <- true
                    printfn "🎥"
                ...
                let status =
                    sprintf "[uptime %s] [fps %s] [bitrate %s]" (v "time") (v "fps") (v "bitrate")

                Console.Write status
                Console.Write "\r"
            | _ -> ()

    exec <| eprintfn "%s" <| handleProgress <| cmd |> ignore

    printfn ""

twitch
    $"""ffmpeg -f gdigrab -framerate 30 -i desktop -c:v libx264 -preset veryfast -maxrate 3000k -bufsize 6000k -pix_fmt yuv420p -g 60 -c:a aac -b:a 128k -f flv {ingestUrl}/{twitchKey}"""
